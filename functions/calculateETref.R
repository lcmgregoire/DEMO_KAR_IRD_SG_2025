calculateETref <- function(x) {
  ...
  for(i in 1:nrow(wthr.df1))
  {
    ...
    ws <- wthr.df1$WS[i]+0.000001 ## WS m/s
    tmax <- wthr.df1$Tmax[i]
    tmin <- wthr.df1$Tmin[i]
    rh <- wthr.df1$RH[i]
    slr.rad <- wthr.df1$SR[i]*0.000001*as.numeric(seq_by) * 60 ### unit from :  W/m² during ...min i to : MJ/s/m² 
    ...
    ETref <- ((0.408*del*Rng) + 0.063189*((900/(24*60/as.numeric(seq_by)))*ws*(es-ea))/(t+273))/(del+0.063189*(1+0.34*ws)) ## 24*60/seq_by : number of time intervals in a day (96 if 15 min timestep, 48 if 30min time step..)  

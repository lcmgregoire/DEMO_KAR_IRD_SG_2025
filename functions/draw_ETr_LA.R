draw_ETr_LA <- function(data) {
  ...
  output_dir <- file.path(full_path, "ETr_LA/")
  ...
  for (i in 2:ncol(data)){
    ...
    file_path <- paste0(output_dir,"/ETr_LA_", safe_name, ".png")
    
    png(file = file_path)
    
    plot<-
      ggplot(data =data, aes(x = TS, data[,i]))+
      ylab("ETr normalized by Leaf area  (mm.time interval-1.cm-2) ")+
      xlab("Timestamp")+
      labs(title = paste("ETr normalized by Leaf area ",colnames(data)[i]))+
      geom_point(aes(y = as.numeric(data[,i])), color = "darkorange")+

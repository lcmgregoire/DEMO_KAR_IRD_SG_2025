draw_ETr_after_conversion <- function(data) {
  ...
  output_dir <- file.path(full_path, "ETr_raw_values_after_conversion")
  ...
  for (i in 1:((ncol(data)-1))){
    ...
    file_name <- paste0("ETr_raw_values_after_conversion_mm_", safe_name, ".png")
  ...
  plot<-
    ...
    xlab("Timestamp")+
    labs(title = paste("ETr ",colnames(data)[i]))+
    geom_point()+

draw_ETr_smooth <- function(data) {
  ...
  data <- cbind(
   ...
   output_dir <- file.path(full_path, "ETr_smooth/")
   dir.create(output_dir, recursive = TRUE, showWarnings = FALSE)
    ...
    for (i in 2:ncol(data)){
      ...
      file_path <- paste0(output_dir,"/ETr_smooth_", safe_name, ".png")

      png(file = file_path)
      
      plot<-
    ggplot(data =data, aes(x = TS, data[,i]))+
    ylab("ETr smooth  (mm.time interval-1) ")+
    xlab("Timestamp")+

draw_ETr_smooth_out_rmd <- function(data) {
  ...
  output_dir <- file.path(full_path, "ETr_smooth_out_rmd")
  ...
  for (i in 2:ncol(data)){
    ...
    file_name <- paste0("ETr_smooth_out_rmd_", safe_name, ".png")
    ...
    plot<-
      ...
      ylab("ETr smmoth  (mm.time interval-1) ")+
      xlab("Timestamp")+
      labs(title = paste("ETr  smooth (non normalized) ",colnames(data)[i]))+
      geom_point(aes(y = as.numeric(data[,i])), color = "darkolivegreen")+

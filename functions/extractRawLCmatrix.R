extractRawLCmatrix <- function(x, y, s, z, inter) {
  ...
  ## create NA when 00:00:00
  for (i in 1:nrow(m.lc.df)) {
    if (is.na(m.lc.df$ts[i])==TRUE){
      m.lc.df$ts =m.lc.df$timestamp  }    }
  ...
  print("This may take a while (depending on the length of the experiment and the number of loadcells). Why don't you go for a coffee? ")
  ...
  # ## pb NA at 00:00:00, no hour created ! 
  for (i in 1:nrow(TS_ALL)) {
    if  (is.na(TS_ALL$TS.n[i])==TRUE)  {
      TS_ALL$TS.n[i] <- as.POSIXct(paste(as.character(date(TS_ALL$TS.n[i+1])), "00:00:00"), tz=attr(TS_ALL$TS.n, "tzone"))  }}
  ...
  for (n in 1:(nrow(sec.dtLen)))
  {
    tmp.lc <- m.lc.df[(m.lc.df$unit == sec.dtLen$sec[n] &
                      ...
                      c("TS.n", "unit", "Weight_g")] #('TS.n', 'unit', 'mass')
  ...
  LC.MAT[LC.MAT$TS > (ymd_hms(paste0(F.dt," ",'23:45:00'))), ]
  
  b <- Sys.time()
  
  print(paste0("Loadcell matrix created in: ", round((b-a), 2), " sec"))
  
  list(LC.MAT.f = LC.MAT, LC_tsmeta = sec.dtLenMETA.FullUnits)
}

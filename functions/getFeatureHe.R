getFeatureHe <- function(x, y, d, p) {
  ...
  # raw.trans[,3]=  raw.trans[,6]
  # colnames(raw.trans)[3]= "Genotype"
  ...
  for (i in 1:length(dates)){
    ...
    features.DF <- as.data.frame(cbind(raw.trans[9:nrow(raw.trans), 1:6], tmp))
    ...
    {
      he.tmp <- features.DF[ , c(4, (f+6))] # 3 is Genotype-column and f+5 is the feature column !! a changer !!

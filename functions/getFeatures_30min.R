getFeatures_30min <- function(x) {
  ...
  # Features to be extracted: "maxET", "curv.maxET", "slope.maxET-6", "slope.07-maxET", "slope.00-07", "slope.19-23:30",
  ...
  colnames(F.mat) <- c("maxET", "slope.maxET.6", "slope.07maxET", "slope.00.07", "slope.19.2330", "curvmaxET", 
  ...
  for(j in 1:ncol(spl.smth.df)){
    ...
    for (i in 1:nrow(spl.smth.whtr.sec.df)) {
      if (is.na( spl.smth.whtr.sec.df[i,12])==TRUE) {
        spl.smth.whtr.sec.df[i,12]<- 0  
      }
      }
    ...
    if( !(max(spl.smth.whtr.sec.df1$Data)==0)) # condition to prevent error columns with all 0 values
    {
      for(i in 1:length(dates)){
        ...
        ## Calculate- "maxET", "slope.maxET-6", "slope.07-maxET", "slope.00-07", "slope.19-23:30", "curvmaxET" 
        #1: maxET
        ...
        #4: slope.00-07
        et.00.loc <- which(hms(data.tmp$time) == hms("00:30:00"))
        ...
        et.23_30.loc <- which(hms(data.tmp$time) == hms("23:30:00"))
        d.tmp3<-data.frame(x3 = 1:(et.23_30.loc - (et.19.loc-1)), y3 = data.tmp$Data[et.19.loc:et.23_30.loc])
        mod3 <- lm(y3~x3, d.tmp3, na.action=na.exclude)
        slope.19.2330 <- mod3$coefficients[2]
        F.mat.list[[j]][i,5] <- slope.19.2330
        ...
        y3.w.loc <-  which(hms(data.tmp$time) == hms("00:30:00"))
        ...
        y5.w.loc <-  which(hms(data.tmp$time) == hms("19:30:00"))
        y6.w.loc <-  which(hms(data.tmp$time) == hms("23:30:00"))
  ...
  featuresRES= list(allFeatures = F.mat.list, weather.DAT = weather.DAT)
